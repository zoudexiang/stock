...
# ======================================================================================
# ✅【修改】20 日涨幅 > 50% 股票策略 + HTML 生成
# ======================================================================================
def generate_rise20_html():
    print("📥 加载 20 日涨幅超 50% 股票数据...")

    # 1. 获取最新一天日期
    last_dt = pd.read_sql("SELECT MAX(dt) AS dt FROM stock_detail", engine).iloc[0]["dt"]

    # 2. 查询 > 50% 股票 + 关联行业 rise_15 实际存储的是 20 日涨幅
    sql = f"""
        SELECT
            s.code,
            s.stock_name,
        ...
        ORDER BY s.rise_15 DESC
    """
    df = pd.read_sql(sql, engine)
    if df.empty:
        print("❌ 暂无 20 日涨幅超 50% 的股票")
        return
    ...
    <body>
        <div class="container">
            <h1 class="title">🚀 20 日涨幅超 50% 强势股 K 线看板</h1>
        ...
        for _, r in sub_df.iterrows():
            ...
            <div class="card">
                ...
                <div class="sub" style="color:red; font-weight:bold;">
                    5日涨幅: {r["rise_5"]:.2f}% ｜ 10日涨幅: {r["rise_10"]:.2f}% ｜ 20日涨幅: {r["rise_15"]:.2f}%
                </div>
    ...
    filename = f"../html/{datetime.now().strftime('%Y-%m-%d')}_20日涨幅超50%股票.html"
