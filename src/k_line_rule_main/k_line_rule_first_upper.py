...
def generate_first_limit_up_html_simple():

    print("📥 精简版 - 加载20日内首次涨停个股数据...")
    ...
    from (
        select 
            ...
            rise
        from stock_detail
        where dt>=date_sub(curdate(), interval 20 day)
            and code in ('002476', '300839', '600281', '605198', '002534', '603565', '600110', '603032', '002580', '301152', '000695', '600186', '000997', '603106', '301630', '603435', '601330', '002412', '002907', '003035', '001376', '603360', '001338', '003030', '601609', '601702', '301626', '300679', '603626', '002947', '605277', '001232', '605258', '301177', '600714', '603389', '603313', '000566', '003036', '002757', '600838', '603618', '002028', '600577', '603045', '600468', '000533', '605566', '002989', '000779', '603637', '002047', '601177', '002686', '603090', '601882', '603201', '603121', '603178', '603007', '002921')
            -- and round(total_market_capitalization / 100000000, 2)>100
    ) a join (
        select 
            code,
    ...
    print("🌍 精简版 - 生成HTML看板文件...")
    html = '''
    <!DOCTYPE html>
    <html lang="zh-CN">
    <head>
        <meta charset="UTF-8">
        <title>精简版 - 20日内首次涨停个股K线看板</title>
        <style>
            *{box-sizing:border-box;margin:0;padding:0;font-family:Microsoft YaHei}
            body{background:#f5f7fa;padding:20px}
            .container{max-width:1900px;margin:0 auto}
    ...
    <body>
        <div class="container">
            <h1 class="title">🔥 精简版 - 20日内首次涨停个股 K线看板（板块内部按距今s天升序，天数越小越靠前）</h1>
            <div class="col-switch">
                <button class="col-btn" onclick="changeColumns(2)">2列</button>
                <button class="col-btn active" onclick="changeColumns(3)">3列</button>
                <button class="col-btn" onclick="changeColumns(4)">4列</button>
    ...
    '''
    out_file = f"../html/{datetime.now().strftime('%Y-%m-%d')}_首次涨停个股看板_精简版.html"
    with open(out_file, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"✅ 文件输出完成：{out_file}")
