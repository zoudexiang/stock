...
-- 倍量回调 - 踩上分歧日 结果表（不指定返回目标天，dt 为分歧天）
CREATE TABLE `stock_detail_calc` (
    `calc_dt` varchar(10) DEFAULT NULL COMMENT '计算日期，格式 yyyy-MM-dd',
    `dt` varchar(10) DEFAULT NULL COMMENT '日期，格式 yyyy-MM-dd',
    `code` varchar(6) DEFAULT NULL COMMENT '股票代码',
    `stock_name` varchar(100) DEFAULT NULL COMMENT '股票名称',
    `price_open` double DEFAULT NULL COMMENT '开盘价',
...
-- 倍量回调 - 踩上分歧日 结果表（dt 为返回目标天）
CREATE TABLE `stock_detail_calc_backtracking` (
  `dt` varchar(10) DEFAULT NULL COMMENT '日期，格式 yyyy-MM-dd',
  `code` varchar(6) DEFAULT NULL COMMENT '股票代码',
  `stock_name` varchar(100) DEFAULT NULL COMMENT '股票名称',
  `price_open` double DEFAULT NULL COMMENT '开盘价',
